...
class MenuItem extends Component {
    ...
    return (
      <div className="MenuItem__menuItemContainer">
        ...
        <div className="MenuItem__menuItemName">
          <ClampLines text={menuItem.name} lines="2" buttons={false} />
        </div>
        <div className="MenuItem__menuItemNutrition">
          {`Cals: ${menuItem.cals} P: ${menuItem.protein}g C: ${
            menuItem.carbs
          }g F: ${menuItem.fat}g`}
        </div>
  ...
  renderMenuItemFooter() {
    ...
    const thumbsUpIconClass = `MenuItem__menuItemPreference${
      menuItem.liked ? ' MenuItem__menuItemPreference--active' : ''
    }`;
    const thumbsDownIconClass = `MenuItem__menuItemPreference MenuItem__menuItemPreference--thumbsDown${
    ...
    return (
      <div className="MenuItem__menuItemFooter">
        <div
          ...
          data-intro={
            this.props.withToolTip
              ? 'Here you can like or dislike meals. This will help us suggest a personalized meal plan for you! 👍'
              : undefined
          }
        >
          <div
            className={thumbsUpIconClass}
            onClick={() => this.props.onLike(menuItem.id)}
          >
            <ThumbIcon />
          </div>
          <div
            className={thumbsDownIconClass}
            onClick={() => this.props.onDislike(menuItem.id)}
          >
            <ThumbIcon />
          </div>
        </div>
        {isInMealPlan ? (
          ...
          <button
            className="button"
            onClick={() => this.props.onAddToMealPlan(menuItem.id)}
          >
            Add
          </button>
        )}
